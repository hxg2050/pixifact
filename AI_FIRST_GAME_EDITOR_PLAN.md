...
- Pixifact 是面向 AI 完整游戏开发的 Scene / UI / 轻场景与项目资产管理层，不是 editor-first 项目。
- Codex、Claude Code 等 coding agent 是主要 AI 入口；Pixifact 不把内置聊天作为主要用户入口。
- Pixifact CLI 是外部 Agent 操作 Scene 的主入口。
- `apps/editor/` 是预览、审查、轻量资产浏览和人工微调工具，不是主要 AI 对话入口。
...
- Editor 对用户表现为 Agent 结果的预览、审查、轻量资产浏览和人工微调工具。
...
- 打开本机文件、调用外部代码编辑器、调用系统默认程序都走 Tauri host。
...
- Editor：打开项目和 Scene，显示 viewport、层级、Inspector、Diff / proposal 状态，提供轻量资产预览、资源引用校验和人工微调。
...
- 文件系统：项目目录、脚本、资源、Component 文件、Scene 文件和轻量资源预览。
...
- 面板中展示当前项目路径和选中文件路径。
- 单击文件只选中并显示详情。
- 单击图片、音频、字体、数据等资源时，在编辑器内做轻量预览或展示摘要。
- 双击图片、音频、字体、数据等具体资源时，调用系统默认程序打开。
- 双击代码文件时，调用外部代码编辑器打开；编辑器不内嵌代码编辑器，也不直接修改代码。
...
桌面壳已进入主线开发，负责本机文件系统、外部代码编辑器打开、系统默认程序打开和最终打包。
...
- 面板标题：文件系统、预制体、Inspector、Viewport、Agent、动作、逻辑、记忆、项目。
- 面向用户和验证流程的错误提示。
...
8. 用户单击资源在文件面板轻量预览，双击具体资源用系统默认程序打开。
9. 用户双击代码文件跳转外部代码编辑器，编辑器本身不修改代码。
10. 用户在 Codex / Claude Code 中提出需求。
11. Agent 通过 Pixifact CLI 生成 SceneCommand 并 dry-run；若非法，根据错误 JSON 修正。
12. 修正到合法后 apply，Viewport、节点树和 Inspector 刷新。
13. 用户可以在 Editor 查看执行结果和关键修正轨迹。
...
- 文件系统面板是左侧资源管理器入口，类似代码编辑器的目录区；第一版展示项目路径、Scene、脚本、Component 文件和资源。
- 代码文件双击跳转外部代码编辑器，编辑器不内嵌代码编辑器，也不直接写磁盘代码文件。
- 所有日常 UI / Scene 微调都通过文件系统、节点树、Viewport、Inspector 和 Agent / CLI 工作流完成。
...
2. 将文件系统、Scene 节点树、Viewport、Inspector、Agent 状态拆成正式面板组件。
3. 将 AI 从 editor 内置 proposal 审核流改为 Agent / CLI-first 的 dry-run、repair loop 和 apply。
4. 将 Inspector 的属性级锁、底部 Add Component、Component 文件拖拽添加接入正式 `SceneDocument` command。
5. 将文件面板接入真实项目路径：轻量资源预览、系统默认程序打开、外部代码编辑器跳转、Scene 双击编辑。
```
