...
export function createDocumentAiProposalContext(document: SceneDocument): AiProposalContext {
    return {
...
function commandsJson(commands: readonly SceneCommand[]) {
...
export function createAiRepairPrompt(
    ...
    return [
        originalPrompt,
        '',
        '上一次返回的 SceneCommand 未通过 Pixifact 校验。请修正 command，并只返回新的 AiProposal JSON。',
...
function dryRunOptions(document: SceneDocument) {
    return {
        locks: document.locks,
        designTokens: document.designTokens,
        actions: document.actions,
...
function applyProposal(document: SceneDocument, proposal: AiProposal) {
...
export async function executeAiPrompt(
    ...
    for (let index = 0; index < options.maxAttempts; index++) {
        ...
        const run = dryRunProposal(document.scene, proposal, dryRunOptions(document));
