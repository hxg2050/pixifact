...
export class WechatPointerEvent {
    altKey = false;
    bubbles = true;
    button = 0;
    buttons = 0;
...
function installCanvasSurface(canvas: WechatCanvas, width: number, height: number) {
...
function createDocumentElement(tagName: string): unknown {
    const wx = wechatApi();
    if (tagName.toLowerCase() === 'canvas') {
        const canvas = wx.createOffscreenCanvas?.({ type: '2d', width: 1, height: 1 }) ?? wx.createCanvas();
        canvas.width = 1;
        canvas.height = 1;
        return canvas;
    }
...
function installGlobals(canvas: WechatCanvas, userAgent: string) {
    ...
    documentTarget.createElement ??= createDocumentElement;
    if (!globals.document) {
        Object.defineProperty(globals, 'document', { configurable: true, value: documentTarget });
    }
    if (!globals.PointerEvent) {
        Object.defineProperty(globals, 'PointerEvent', { configurable: true, value: WechatPointerEvent });
    }
...
function createContext(canvas: WechatCanvas) {
    const attributes = {
        alpha: false,
        antialias: true,
    ...
    throw new Error('The current WeChat Mini Game runtime does not provide WebGL.');
}
...
export function createWechatRuntime(): WechatRuntime {
    ...
    const resolution = Math.min(2, Math.max(1, Number(info.pixelRatio)));
    const userAgent = ['WeChatMiniGame', info.platform, info.system, info.model]
        .filter(Boolean)
        .join('/');
    installCanvasSurface(canvas, width, height);
    installGlobals(canvas, userAgent);
