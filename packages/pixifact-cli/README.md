...
其他命令是辅助入口：`scene create` 用于新建 Scene，`node inspect` 用于已知 locator 的节点详情，`editor context` 用于 Editor 运行时读取当前 Scene 和 selection。不要使用 `scene get`，file mode 只保留 `scene inspect`。旧 `live ...` 命令已经删除。
...
同一项目重复运行 `pixifact editor` 会复用现有 Host；一个 Host 同时只接受一个浏览器 Editor 页面。Editor 已同步时可运行 `pixifact editor context` 获取只读上下文。
