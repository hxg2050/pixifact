...
CLI 会启动只绑定 `127.0.0.1` 的本地 Bun 服务并打开系统浏览器。Editor 从项目内现有 `.scene` 建立层级、Pixi 预览和 Inspector。画布左上角可切换拖动画布、移动节点和调整大小工具；属性输入实时更新预览。默认手动保存，点击“保存”或按 Ctrl/Cmd+S 写入文件，也可在左下角设置中开启自动保存。多个 Scene 可在同一 Editor 的标签中切换，各自保留未保存修改。Scene 写入携带当前文件版本，版本不一致时提示比较草稿与磁盘版本，不会静默覆盖外部 Agent 的修改。
