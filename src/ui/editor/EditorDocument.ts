...
import { isEditorProjectState } from "./EditorProjectState";
import type { EditorProjectState } from "./EditorProjectState";
...
export interface EditorDocumentEventMap {
    changed: [result: CommandResult];
    loaded: [prefab: PrefabSpec];
...
export class EditorDocument {
    public prefab: PrefabSpec;
    public selection: EditorSelection = emptySelection;
    public dirty = false;
    public emitter = new EventEmitter<EditorDocumentEventMap>();
    ...
    constructor(prefab: PrefabSpec) {
        this.prefab = clonePrefab(prefab);
    }

    apply(command: EditorCommand, source: OverrideSpec['source'] = 'manual'): CommandResult {
        const result = this.commandStack.execute(this.prefab, command, {
    ...
    undo() {
        const result = this.commandStack.undo(this.prefab, {
            actions: this.actions.length > 0 ? this.actions : undefined,
        });
        if (result?.ok) {
            this.dirty = true;
    ...
    redo() {
        const result = this.commandStack.redo(this.prefab, {
            actions: this.actions.length > 0 ? this.actions : undefined,
        });
        if (result?.ok) {
            this.dirty = true;
    ...
    private ensureSelectionValid() {
        ...
        if (!findNode(this.prefab, this.selection.node)) {
    ...
    serialize() {
        return JSON.stringify(this.prefab, null, 2);
    }
    ...
    load(json: string | PrefabSpec | EditorProjectState) {
        const parsed = typeof json === 'string'
            ? JSON.parse(json) as PrefabSpec | EditorProjectState
            : json;

        if (isEditorProjectState(parsed)) {
            this.loadState(parsed);
            return;
        }

        this.prefab = clonePrefab(parsed);
        this.selection = emptySelection;
        ...
        this.rebuildPreview();
        this.emitter.emit('loaded', this.prefab);
    }

    loadState(state: EditorProjectState) {
        this.prefab = clonePrefab(state.prefab);
    ...
    rebuildPreview(context = this.previewContext, parent = this.previewParent) {
        ...
        this.preview = createRuntimePreview(this.prefab, parent, context);
    ...
    getInspectorModel(node = this.selection.type === 'none' ? undefined : this.selection.node): InspectorNodeModel | undefined {
        ...
        return buildInspectorModel(this.prefab, node);
    }
