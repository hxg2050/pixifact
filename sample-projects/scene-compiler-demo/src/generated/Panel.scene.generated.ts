...
export function mountPanelScene(root: Container) {
  const __pixifactSlots: Record<string, Container> = {};
  root.width = 520;
  root.height = 280;
  const background = new Graphics();
  background.label = "background";
  background.roundRect(0, 0, 520, 280, 18).fill(1647157);
  root.addChild(background);
  const headerRule = new Graphics();
  headerRule.label = "headerRule";
  headerRule.position.set(28, 72);
  headerRule.rect(0, 0, 464, 1).fill(3756143);
  root.addChild(headerRule);
  const titleText = new Text({ text: "Panel", style: { fontSize: 24, fontWeight: "800", fill: 16777215 } });
