...
- Editor 是能力增强：preview、assets、diagnostics、current selection、后续只读 context、最近一次外部 `.scene` 刷新/校验结果。
- 旧 `live ...` CLI 与固定端口 bridge 已删除；后续 Editor context 不提供 mutation action。
