...
- Bun 项目根目录使用 `bunfig.toml` 的 `env = false` 禁止 Bun 预加载 dotenv，确保 env 只由 Vite 按 mode 规则读取；脚手架和示例默认生成该文件。
...
- version 2 使用根级映射表达 remote pack，避免恢复 per-target 重复配置：
...
- `remoteResourcePacks` 必须引用已声明 pack；列入映射的 pack 在所有平台使用 HTTPS URL，不进入 Web 或小游戏产物；其他 pack 在 Web 由 Vite emit，在小游戏成为分包。
...
- 按已确认的 `remoteResourcePacks` schema 实现。
...
- [x] version 2 project config 接受固定目录和 resource pack names，拒绝旧 targets schema。
- [x] `--mode game1` 加载 `.env.game1`；`.env.[mode].local` 和已有 process env 遵循 Vite 优先级。
- [x] Bun 项目通过 `bunfig.toml` 禁止自动 dotenv，启动 CLI 时不破坏 Vite mode-specific env 行为。
- [x] `VITE_PLATFORM` 缺失或非法时 validate/dev/build 失败。
- [x] Web 忽略 `VITE_APP_ID`；微信/抖音缺少 AppID 时失败，并将有效值写入输出配置。
- [x] `--target` 不再出现在 CLI help 或 public command contract，未知参数统一失败。

### Platform packages and Application

- [x] 平台包 import 不读取 `wx` / `tt`，在非目标 dev module evaluation 中安全。
- [x] `createApplication()` 返回原生 Pixi `Application`，options 透传符合契约。
- [x] 微信/抖音 canvas、renderer、input、lifecycle 和 destroy cleanup 通过 runtime mock。
- [x] 抖音 target 保持 ES2018，微信保持已验证 target；真实产物不出现目标不支持语法。
- [x] 删除旧 platform exports 后，仓库内无旧 import。

### Vite build and tree-shaking

- [x] Web、微信、抖音从同一 `src/main.ts` 构建成功。
- [x] mini 输出单一 `game.js` IIFE，无 HTML 依赖、无运行时 dynamic import。
- [x] Web bundle 不含微信/抖音 marker、`wx` / `tt` adapter 和包代码。
- [x] 微信 bundle 不含抖音 marker；抖音 bundle 不含微信 marker。
- [x] 静态 import 的平台专属函数在 env 常量 false 时从 production bundle 删除。
- [x] 用户 `vite.config.ts` 的安全配置可生效；小游戏 invariant 不能被意外破坏。
- [x] TS、Scene、resource pack、native config 改动由 Vite watch 触发正确重建，输出目录不会触发自循环。

### Pixi Assets and resource delivery

- [x] `pixifact:assets` 导出 Pixi manifest，不执行 `Assets.init()`。
- [x] `Assets.load()` 加载主包纹理和 JSON，`Assets.loadBundle()` 加载完整 pack。
- [x] Scene-generated prepare 与业务资源共用同一 Assets resolver/manifest。
- [x] 微信/抖音首次读取分包资源前自动 load subpackage；并发请求只调用一次平台 API。
- [x] 分包加载失败后下一次请求可重试，不缓存 rejected promise。
- [x] remote pack 映射 HTTPS URL 且不进入代码包；HTTP URL validation 失败。
- [x] bitmap font、音频和关联资源保持包内相对路径。
- [x] 生成的 `game.json` 使用微信 `subpackages` / 抖音 `subPackages` 正确字段。
- [x] 主包、各分包、总包报告与输出字节一致，平台限制继续生效。

### Packaging and release

- [x] Web-only tarball install 不拉取可选平台包。
- [x] scoped packages 可公开 pack/publish，exports 与 types 完整。
- [x] fixed group、version sync、publish workflow 和 release smoke 包含两个新包。
- [x] create-pixifact 新项目可安装、validate、Web build，并能按需安装小游戏包。
...
- [x] 创建实施计划。
- [x] 确认 remote resource pack version 2 schema。
- [x] Phase 0：行为测试。
- [x] Phase 1：平台包拆分。
- [x] Phase 2：统一 Vite plugin/build。
- [x] Phase 3：CLI/env/project schema。
- [x] Phase 4：Pixi Assets/resource delivery。
- [x] Phase 5：sample/scaffold/docs。
- [x] Phase 6：release integration。
- [x] 完整自动化验证；开发者工具/真机验收保留为发布前人工步骤。
...
5. Phase 4 按已确认的 `remoteResourcePacks` 根级映射实现。
...
Done:
- Web、微信、抖音已由一份 `src/main.ts` 和统一 Vite plugin 构建；两个小游戏平台包可选安装。
- `pixifact.project.json` version 2、Vite mode/env、PixiJS Assets、远程 pack、小游戏分包和包体报告已落地。
- CLI 已删除 `--target` 契约；未知参数统一失败；Bun 项目通过 `bunfig.toml` 交给 Vite 独占 env 加载。
- 平台包、Application、tree-shaking、Vite watch、Scene 生成稳定性、脚手架、示例、文档和 release smoke 均有自动化验证。
- Changesets fixed group 已确认五包 minor bump 到 `0.7.0`。
- 抖音 ES2018 修复后的产物已由用户完成真机运行验证。

Current State:
- 自动化实施与最终验证完成，当前准备提交。
- `bun run release:check` 已通过 23 个测试文件、292 项测试、三包构建、Editor 构建、五包内容检查和真实安装 smoke。
- `bun run editor:typecheck`、Changesets `0.7.0` version status、旧 API 搜索和 `git diff --check` 均通过。
...
Next:
1. 提交统一 Vite 平台构建变更。
2. 发布前按 Manual acceptance 在 Web、微信开发者工具和抖音开发者工具复验新架构产物。
