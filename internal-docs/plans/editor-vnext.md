...
- 资产面板直接从 `project.files` 构建项目相对目录树，只保留 Scene 和图片；首次打开时目录默认折叠，仅展开当前 Scene 的祖先路径。用户调整后的展开集合保存到 Git 忽略的 `.pixifact/editor/ui-state.json`，并由本地 Editor 服务跨重启恢复；文件夹优先排序，不新增 Pinia 项目状态或资源管理入口。
