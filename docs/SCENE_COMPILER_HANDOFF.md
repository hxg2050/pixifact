...
`@scene` 当前已具备普通 TS 运行时能力：它会在用户 constructor 执行后挂载已注册的 `.scene`，绑定 `@part` 和 `@slot`，应用 `@prop` 默认值，然后调用 `onSceneReady()`。因此 constructor 中不要访问 `@part`；需要访问内部节点的初始化逻辑放在 `onSceneReady()`。
