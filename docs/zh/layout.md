...
- `BitmapLabel extends Control`，使用 bitmap font 渲染，并保持与 `Label` 相同的盒子语义。
...
- 画布左上角可切换“拖动画布”“移动节点”“调整大小”；默认“调整大小”。
- “拖动画布”用左键平移视图，不编辑节点；任何工具下都可按空格键、中键或右键按住拖动画布。右键拖动也适用于系统映射为右键的触控板双指按压；滚轮仍用于缩放。
- “移动节点”可选择并拖动已选中节点，不显示 resize 手柄。选中节点的变换原点会显示红色 X 箭头、绿色 Y 箭头和交点方块；拖动箭头只沿对应轴移动，拖动方块可自由移动。手柄大小不随画布缩放变化。
- “调整大小”保留拖动已选中节点和八个手柄调整尺寸。
