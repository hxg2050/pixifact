...
- `packages/platform-wechat/`：可选微信小游戏 PixiJS adapter、输入、生命周期和资源访问。
- `packages/platform-douyin/`：可选抖音小游戏 PixiJS adapter、输入、生命周期和资源访问。
- `packages/pixifact/src/platform/minigame/`：两个小游戏平台包复用的内部公共 runtime。
...
维护这个 skill 时只保留下游游戏项目里的 Agent 工作流、约束和随 skill 安装后仍可用的离线参考，例如：
...
- 在 `references/scene-objects.md` 中维护下游 AI 离线可读的 `.scene` 对象和属性速查。
...
- 完整 `.scene` 对象清单、属性表或语法大全不要塞进 `SKILL.md` 正文；需要随 skill 安装时，放到 `skills/pixifact/references/scene-objects.md`。

这些内容的权威位置是：

- 下游 Agent `.scene` 工作流：`docs/zh/agent-scene-authoring.md` 和 `docs/en/agent-scene-authoring.md`。
- `.scene` 对象和属性：`docs/zh/scene-objects.md` 和 `docs/en/scene-objects.md`；skill 离线副本为 `skills/pixifact/references/scene-objects.md`。
