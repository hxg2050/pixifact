...
- `EditorApp.vue`：顶层文档绑定、Scene 打开、Undo / Redo、保存和同步状态。

项目数据以 compiler `.scene` 文件为 source of truth，Pinia 只保存轻量 UI 状态。
层级操作直接提交 Compiler Scene Command 并进入 Undo / Redo；默认手动保存，左侧栏设置可开启自动保存。节点通过整行 Pointer Events 拖动，不依赖浏览器原生 Drag and Drop。结构变化由预览层替换 Scene root，不重建 Pixi Application 或 Canvas。
