...
- `AgentPanel.tsx`：Agent / CLI 状态与当前项目、当前 Scene 的命令引导；主路径是帮助外部 Agent 直接编辑 `.scene` 后运行 inspect / validate / compile / build。面板也支持粘贴 `.scene proposal`，作为需要显式审查或防覆盖时的可选保险路径。
